	...
	$.Redactor.prototype.properties = function()
	{
		return {
			...
			getTemplate: function()
			{
				var that = this;
				 ...
				 + '<div class="modal-section" id="redactor-modal-properties">'
				 	+ '<section>'
				 		+ '<label id="modal-properties-id-label">' + that.lang.get('id') + '</label>'
				 		+ '<input type="text" id="modal-properties-id" />'
				 	+ '</section>'
				 	+ '<section>'
					 	+ '<label id="modal-properties-class-label">' + that.lang.get('class') + '</label>'
					 	+ '<input type="text" id="modal-properties-class" />'
					 + '</section>'
					+ '<section>'
						+ '<button id="redactor-modal-button-action">' + that.lang.get('save') + '</button>'
						+ '<button id="redactor-modal-button-cancel">' + that.lang.get('cancel') + '</button>'
					+ '</section>'
				 + '</div>';
			},
			setup: function()
			...
			show: function()
			{
				...
				this.modal.load('properties', that.lang.get('properties'), 600);

				var button = this.modal.getActionButton().text(that.lang.get('save'));
